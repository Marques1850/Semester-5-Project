...
export default class ThumbRaiser {
    constructor(generalParameters, audioParameters, cubeTexturesParameters, mazeParameters, playerParameters, ambientLightParameters, directionalLightParameters, spotLightParameters, flashLightParameters, shadowsParameters, fogParameters, collisionDetectionParameters, fixedViewCameraParameters, firstPersonViewCameraParameters, thirdPersonViewCameraParameters, topViewCameraParameters, miniMapCameraParameters, buildingCodeparam) {
        ...
        this.buildingCode = buildingCodeparam;

        // Set the game state
        this.gameRunning = false;

        //Set up debounces for interactions
        this.elevPopupDebounce = false;
        this.passagePopupDebounce = false
        ...
        //Build the Raycaster
        this.raycaster = new Raycaster(this.activeViewCamera, window, this.scene);
    }
        ...
        if (!this.gameRunning) {
            ...
            if (!this.animations.actionInProgress) {
                // Check if the player found the exit
                    ...
                    let collisionReturn = this.maze.collision(this.collisionDetectionParameters.method, position, this.collisionDetectionParameters.method != "obb-aabb" ? this.player.radius : this.player.halfSize, directionRad - this.player.defaultDirection);
                    if (collisionReturn.returnValue) {
                        if (collisionReturn.collisionType == "wall") {
                        this.audio.play(this.audio.deathClips, false);
                        this.animations.fadeToAction("Death", 0.2);

                        } else if (collisionReturn.collisionType == "elevator") {
                            // Create a new HTML element for the pop-up
                            //PopUp();
                            if (this.elevPopupDebounce === true){
                                return;
                            }
                            let {popup, dropdown, button} = this.popUp.popupElevator();
                            this.elevPopupDebounce = true;
                            // Fetch the floors from the server
                            let url = 'http://localhost:3000/api/floor/listFloorsWithElevators';
                            let codeStr = this.buildingCode;
                            let params = new URLSearchParams();
                            params.append('buildingCode', codeStr);
                            ...
                            button.addEventListener('click', () => {
                                ...
                                // Call a function in your TypeScript code
                                let url = 'http://localhost:3000/api/floor/editPlayerPosition';

                                let orientation=0;
                    
                              
                                let params = new URLSearchParams();
                                params.append('buildingCode',this.buildingCode);
                                params.append('floorName', selectedItem);
                                params.append('X', collisionReturn.elevatorPosition[0]);
                                params.append('Y', collisionReturn.elevatorPosition[1]);
                                ...
                                .catch(error => console.error('Error:', error));
                                  ...
                                  let path = "./../../assets/mazes/" + selectedItem + ".json";
                                  
                                  let finalpath="/plant?url=" + encodeURIComponent(path) + "&buildingCode=" + encodeURIComponent(this.buildingCode);
                
                                // Create a new HTML element for the loading text
                                let loadingText = document.createElement('p');

                                // Set the content of the loading text
                                loadingText.innerHTML = "Loading...";

                                // Append the loading text to the pop-up
                            ...
                            // Append the pop-up to the body of the document
                            document.body.appendChild(popup);

                        } else if (collisionReturn.collisionType == "passage") {
                            // Create a new HTML element for the pop-up
                            ...
                            this.elevPopupDebounce = true;
                            let {popup} = this.popUp.popupPassage();
                            popup.innerHTML = "Enter the passage: " + collisionReturn.passageFloor;

                            let path = "./../../assets/mazes/" + collisionReturn.passageFloor + ".json";
                                  this.buildingCode = "Edif"+collisionReturn.passageFloor.slice(collisionReturn.passageFloor.length-1);
                            let finalpath="/plant?url=" + encodeURIComponent(path) + "&buildingCode=" + encodeURIComponent(this.buildingCode);
                           
                            let floorPositions;
                            let floorsPassages = fetch(path)
                            .then(response => response.json())
                            .then(json => json.maze)
                            .catch(error => console.error('Error:', error));

                            let decodedPath = decodeURIComponent(window.location.href);
                            let components = decodedPath.split("/");
                            let jsonFileName = components[components.length - 1];
                            let collisionReturnPassageFloor = jsonFileName.replace(".json", "");
                            let component = collisionReturnPassageFloor.split("&");
                            let currentFloor=component[0];
                            
                             
                            let orientation=90;

                            floorsPassages.then(floorsPassages => {
                                for(let i=0; i<floorsPassages.exitFloor.length; i++){
                                   
                                  if(floorsPassages.exitFloor[i] == currentFloor){
                                        floorPositions = floorsPassages.exitLocation[i];
                                        if(floorPositions[1] == floorsPassages.size.width){
                                            orientation=-90;
                                            floorPositions[1] = floorPositions[1] - 1;
                                        }
                                        if(floorPositions[0] == floorsPassages.size.depth){
                                            orientation=180;
                                            floorPositions[0] = floorPositions[0] - 1;
                                        }
                                        break;
                                  }
                                }
                                ...
                                let params = new URLSearchParams();
                                params.append('buildingCode',this.buildingCode);
                                params.append('floorName', collisionReturn.passageFloor);
                                params.append('X', floorPositions[0]);
                                params.append('Y', floorPositions[1]);
                                params.append('orientation', orientation);
                                let response;
                                fetch(url + '?' + params.toString())
                                    .then(response => response.json())
